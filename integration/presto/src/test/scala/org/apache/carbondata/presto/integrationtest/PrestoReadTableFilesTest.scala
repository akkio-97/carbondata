...
class PrestoReadTableFilesTest extends FunSuiteLike with BeforeAndAfterAll with BeforeAndAfterEach{
  ...
  def buildComplexTestForSingleLevelArray(): Any = {
    ...
    val source = new File(this.getClass.getResource("/").getPath + "../../" + "/temp/table1")
      .getCanonicalPath
    val srcDir = new File(source)
    val destination = new File(
      this.getClass.getResource("/").getPath + "../../" + "/target/store/sdk_output/files/")
      .getCanonicalPath
  ...
  def buildComplexTestFor2LevelArray(): Any = {
    ...
    import java.io.IOException
    val source = new File(this.getClass.getResource("/").getPath + "../../" + "/temp/table2")
      .getCanonicalPath
    val srcDir = new File(source)
    val destination = new File(
      this.getClass.getResource("/").getPath + "../../" + "/target/store/sdk_output/files2/")
      .getCanonicalPath
    val destDir = new File(destination)
    try FileUtils.copyDirectory(srcDir, destDir)
    catch {
  ...
  def buildComplexTestFor3LevelArray(): Any = {
    writerPath = storePath + "/sdk_output/files3"
    FileUtils.deleteDirectory(new File(writerPath))
    createComplexTableFor3LevelArray
    import java.io.IOException
    val source = new File(this.getClass.getResource("/").getPath + "../../" + "/temp/table3")
      .getCanonicalPath
    val srcDir = new File(source)
    val destination = new File(
      this.getClass.getResource("/").getPath + "../../" + "/target/store/sdk_output/files3/")
      .getCanonicalPath
    val destDir = new File(destination)
    try FileUtils.copyDirectory(srcDir, destDir)
    catch {
...
//    buildComplexTestForSingleLevelArray()
  ...
  */
  ...
  test("test 2-level for array of int, bigInt") {
    buildComplexTestFor2LevelArray()
    val actualResult: List[Map[String, Any]] = prestoServer
  ...
  test("test 3-level array for int, BigInt, Real, Double, String and Boolean columns") {
    buildComplexTestFor3LevelArray()
    val actualResult: List[Map[String, Any]] = prestoServer
